...
exports.convertToAutoLayout = void 0;
const convertGroupToFrame_1 = require("./convertGroupToFrame");
// Get the element with the highest occurrence in an array. https://stackoverflow.com/a/20762713
const mostFrequentArrayEl = (arr) => {
...
const convertToAutoLayout = (node) => {
    ...
    if (('layoutMode' in node && node.layoutMode === 'NONE' && node.children.length > 0) || node.type === 'GROUP') {
        ...
        if (node.type === 'GROUP') {
            node = (0, convertGroupToFrame_1.convertGroupToFrame)(node);
        }
        if (direction === 'NONE' && node.children.length === 1) {
        ...
        node.primaryAxisAlignItems = mostFrequentArrayEl(primaryDirection);
        // @ts-ignore it is never going to be undefined.
        node.counterAxisAlignItems = mostFrequentArrayEl(counterDirection);
        node.counterAxisSizingMode = 'FIXED';
        node.primaryAxisSizingMode = 'FIXED';
    }
    return node;
};
exports.convertToAutoLayout = convertToAutoLayout;
/**
