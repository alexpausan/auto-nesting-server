...
exports.getBoundingRect = exports.notEmpty = exports.convertIntoAltNodes = exports.frameNodeToAlt = exports.convertSingleNodeToAlt = void 0;
const convertNodesOnRectangle_1 = require("./convertNodesOnRectangle");
...
const frameNodeToAlt = (node, altParent = null) => {
    ...
    const altNode = new altMixins_1.AltFrameNode();
    altNode.id = node.id;
    ...
    altNode.children = (0, exports.convertIntoAltNodes)(node.children, altNode);
    return (0, convertToAutoLayout_1.convertToAutoLayout)((0, convertNodesOnRectangle_1.convertNodesOnRectangle)(altNode));
};
exports.frameNodeToAlt = frameNodeToAlt;
// auto convert Frame to Rectangle when Frame has no Children
const frameToRectangleNode = (node, altParent) => {
    const newNode = new altMixins_1.AltRectangleNode();
    newNode.id = node.id;
    ...
    const mapped = sceneNode.map((node) => {
        if (node.type === 'RECTANGLE' || node.type === 'ELLIPSE') {
            ...
            if (node.type === 'RECTANGLE') {
                altNode = new altMixins_1.AltRectangleNode();
                convertRectangleCorner(altNode, node);
            }
            else {
                altNode = new altMixins_1.AltEllipseNode();
            }
            altNode.id = node.id;
            altNode.name = node.name;
            if (altParent) {
            ...
            const altNode = new altMixins_1.AltRectangleNode();
            altNode.id = node.id;
            altNode.name = node.name;
            if (altParent) {
                altNode.parent = altParent;
            ...
            return (0, exports.frameNodeToAlt)(node, altParent);
        }
            ...
            if (node.children.length === 1 && node.visible !== false) {
                ...
                return (0, exports.convertIntoAltNodes)(node.children, altParent)[0];
            }
            const iconToRect = iconToRectangle(node, altParent);
            if (iconToRect != null) {
                return iconToRect;
            }
            const altNode = new altMixins_1.AltGroupNode();
            altNode.id = node.id;
            altNode.name = node.name;
            if (altParent) {
                altNode.parent = altParent;
            ...
            return (0, convertNodesOnRectangle_1.convertNodesOnRectangle)(altNode);
        }
        else if (node.type === 'TEXT') {
            const altNode = new altMixins_1.AltTextNode();
            altNode.id = node.id;
            altNode.name = node.name;
            if (altParent) {
                altNode.parent = altParent;
...
exports.convertIntoAltNodes = convertIntoAltNodes;
...
const convertLayout = (altNode, node) => {
    ...
    if (node.rotation !== undefined && Math.round(node.rotation) !== 0) {
        const boundingRect = (0, exports.getBoundingRect)(node);
