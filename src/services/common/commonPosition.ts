...
import { FrameNode, ComponentNode, ChildrenMixin } from 'figma-plugin-ds';
...
export const commonIsAbsolutePosition = (
  ...
  if (
    ...
    ("inferredAutoLayout" in node.parent ||
      "inferredComponentLayout" in node.parent)
  ) {
    if (
      ("inferredAutoLayout" in node.parent &&
        node.parent.inferredAutoLayout !== null) ||
      ("inferredComponentLayout" in node.parent &&
        node.parent.inferredComponentLayout !== null)
    ) {
      return false;
    }
  }
